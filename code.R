...
probeExpr <- read.table('12.DEAREGene boxpolt/GSE54236_probeExpr.txt', header = T, sep = '\t', check.names = F)
GPL <- read.table('12.DEAREGene boxpolt/GPL6480-9577proble-symbol.txt', header = T, sep = '\t', check.names = F)
symbolExpr <- merge(GPL, probeExpr, by = 'ID_REF')
symbolExpr <- na.omit(symbolExpr)
symbolExpr <- symbolExpr[, -1]
range(symbolExpr[,-1])
write.table(symbolExpr, file = '12.DEAREGene boxpolt/GSE54236_symbolExpr.txt', sep="\t", row.names = F, quote = F)

gene<-read.table("12.DEAREGene boxpolt/GSE54236_symbolExpr.txt",sep="\t",header=T, check.names = F)
data3 <- aggregate( . ~ id, data=gene, max)  
range(data3[,-1])
...
data4 <- log2(data3)
range(data4)
write.table(cbind(id=row.names(data4), data4),file="12.DEAREGene boxpolt/GSE54236_symbolExprUniMax.txt",sep="\t",quote = F,row.names=F)
